...
class Trigger extends Collider {
  ...
  update(other: Collider | RigidBody): void {
    if (this.collidesWith(other)) { // Is inside
      if (!includes(this.particles, other)) { // Is not in the list
      ...
      if (!includes(this.particles, other)) { // Is outside but is in the list
        this.particles.slice(this.particles.indexOf(other), 1)
